...
# ===导入数据
df = pd.read_csv('price_test.csv', encoding='gbk', parse_dates=['TRADE_DT'], low_memory=False)  # 从csv文件中读取整理好的所有股票数据
#数据合并处理

for i in range(1, 6):
    # 读取CSV文件
    df_temp = pd.read_csv(f'factor{i}_test.csv', encoding='gbk', parse_dates=[0], low_memory=False)
